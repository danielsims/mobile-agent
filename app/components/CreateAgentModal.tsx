...
const FALLBACK_MODEL: ProviderModelOption = {
  value: 'auto',
...
export function CreateAgentModal({
  ...
  // --- Step 2: Model ---
  if (step === 'model') {
    const models = modelsByType[selectedType] && modelsByType[selectedType].length > 0
      ? modelsByType[selectedType]
      : [FALLBACK_MODEL];
              ...
              models.map(({ value, label, note }) => (
                ...
                >
                  <View style={styles.modelRowInfo}>
                    <Text style={styles.typeLabel}>{label}</Text>
                    {note ? <Text style={styles.modelNote}>{note}</Text> : null}
                  </View>
                  <View style={styles.chevron}>
                    <View style={[styles.chevronArrow, { borderColor: '#444' }]} />
                  </View>
