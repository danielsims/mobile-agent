...
export function CreateAgentModal({
  ...
  const handleTypeSelect = useCallback((type: AgentType) => {
    ...
    setStep('model');
  }, [onRequestModels, onRequestProjects]);
  ...
  const handleSubmit = useCallback((projectId: string | null, worktreePath: string | null) => {
    onCreate({
      ...
      projectId,
      worktreePath,
      model: selectedType === 'terminal' ? '' : effectiveModel,
      prompt: '',
    });
    onClose();
  ...
  const stepTitle = step === 'type' ? 'New Session' : step === 'model' ? 'Select Model' : 'Select Project';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {step === 'project' && (
          <div className="create-modal-body">
            {/* No project option (non-terminal only) */}
